...
{
    [Authorize]
    public class PaymentMTController : Controller
    {
        private BillingContext _billingsoftware;
        private readonly IConfiguration _configuration;


        public PaymentMTController(BillingContext billingsoftware, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> PaymentAction(PaymentTableViewModel model, string buttonType, string selectedSlotId, string billId, string branchID, string billDate, string billValue, string CloseBy, string CloseDate)
        {
            ...
            if (buttonType == "GetBill")
            {
                ...
                if (billDetails != null)
                {
                    ...
                    obj.closedBy = billDetails.CloseBy;
                    obj.ClosedDate = billDetails.CloseDate;
                }
                else
                {
            ...
            if (buttonType == "DeletePaymentDetail")
            {
                ...
                if (string.IsNullOrEmpty(selectedSlotId))
                {
                    ...
                    var getbillvalue = await _billingsoftware.SHbillmaster.Where(x => x.BillID == billId && x.BranchID == model.BranchID).FirstOrDefaultAsync();
                    if (getbillvalue != null)
                        ViewBag.Total = getbillvalue.NetPrice.ToString();
                ...
                var exbilltotal = await _billingsoftware.SHbillmaster.Where(x => x.BillID == billId && x.BranchID == model.BranchID).FirstOrDefaultAsync();
                if (exbilltotal != null)
                    model.Balance = exbilltotal.NetPrice.ToString();

                ViewBag.Total = exbilltotal.NetPrice.ToString();
            ...
            if (buttonType == "Save")
            {
                ...
                /*  if(model.Viewpayment==null && model.Viewpayment.Any())
               {
                ...
                if ((decimal)totalpayamount > Convert.ToDecimal(billAmount))
                {
                ...
                var exbilltotal = await _billingsoftware.SHbillmaster.FirstOrDefaultAsync(x => x.BillID == billId && x.BranchID == model.BranchID);
                if (exbilltotal != null)
                    model.Balance = exbilltotal.NetPrice.ToString();

                var updpayment = _billingsoftware.SHbillmaster.FirstOrDefault(x => x.BillID == billId && x.BranchID == model.BranchID);
                if (updpayment != null)
                {
                    updpayment.CloseBy = CloseBy;
                    updpayment.CloseDate = CloseDate;
            ...
            if (buttonType == "AddPayment")
            {
                ...
                var exbill = await _billingsoftware.SHbillmaster
                ...
                if (exbill != null)
                {
                    ...
                    model.Balance = exbill.NetPrice.ToString();
                    ...
                    ViewBag.Total = exbill.NetPrice.ToString();
                    billValue = ViewBag.Total;
